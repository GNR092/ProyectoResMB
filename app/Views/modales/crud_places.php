...
<div id="pantalla-lista-places" class="p-6 bg-white rounded-xl shadow-md">
    ...
    <div id="form-filtros-places" class="flex flex-col sm:flex-row sm:items-center gap-4 mb-4 text-sm">
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full border border-gray-300 rounded-lg table-fixed text-sm">
            <thead>
            <tr>
                <th class="w-1/5 px-3 py-2 border-b text-left">Nombre Corto</th>
                <th class="w-1/5 px-3 py-2 border-b text-left">Nombre Completo</th>
                <th class="w-1/5 px-3 py-2 border-b text-left">Razón Social</th>
                <th class="w-1/5 px-3 py-2 border-b text-left">Segmento</th>
            ...
            <?php if (!empty($places)): ?>
                <?php foreach ($places as $index => $place): ?>
                    <tr data-id="<?= $place['ID_Place'] ?>"
                        ...
                        data-id-segmento="<?= esc($place['id_segmento'] ?? '') ?>"
                        ...
                        <td class="px-2 py-2 border-b align-top text-center acciones">
                            <div class="flex flex-col items-center space-y-1 h-full justify-center">
                                <a href="#" id="btn-editar-places-<?= $place['ID_Place'] ?>" class="btn-editar text-green-600 hover:text-green-800" data-id="<?= $place['ID_Place'] ?>">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/>
                                    </svg>
                                </a>
                                <a href="#" id="btn-eliminar-places-<?= $place['ID_Place'] ?>" class="btn-eliminar text-red-600 hover:text-red-800" data-id="<?= $place['ID_Place'] ?>">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
            ...
            <?php else: ?>
                <tr>
                    <td colspan="5" class="px-3 py-4 text-center text-gray-500">No hay lugares registrados</td>
                </tr>
            <?php endif; ?>
            </tbody>
        </table>
    ...
    <form id="form-agregar-places" class="space-y-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div class="flex flex-col">
                ...
                <select name="ID_RazonSocial" id="ID_RazonSocial" class="w-full px-3 py-2 border rounded-lg bg-white" required>
            ...
            <div class="flex flex-col">
                <label for="id_segmento" class="mb-1 font-medium">Segmento de Negocio</label>
                <select name="id_segmento" id="id_segmento" class="w-full px-3 py-2 border rounded-lg bg-white">
                    <option value="">-- Seleccione Segmento (Opcional) --</option>
                    <?php foreach ($segmentos as $s): ?>
                        <option value="<?= $s['id'] ?>" data-id-rs="<?= $s['id_razon_social'] ?>"><?= esc($s['nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
    ...
    <form id="form-editar-places" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            <div class="flex flex-col">
                ...
                <select name="ID_RazonSocial" id="editar-ID_RazonSocial" class="w-full px-3 py-2 border rounded-lg bg-white" required>
                    <?php foreach ($razones_sociales as $rs): ?>
                        <option value="<?= $rs['ID_RazonSocial'] ?>"><?= esc($rs['Nombre']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="flex flex-col">
                <label for="editar-id_segmento" class="mb-1 font-medium">Segmento de Negocio</label>
                <select name="id_segmento" id="editar-id_segmento" class="w-full px-3 py-2 border rounded-lg bg-white">
