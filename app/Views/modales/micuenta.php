...
<div x-data="Account()" id="micuenta" class="p-6 bg-white rounded-xl shadow-md">
    ...
    <form id="form-update-user" @submit.prevent="CambiarNombre()" x-ref="xUserForm" class="mb-8 p-6 border border-gray-200 rounded-lg shadow-sm">
        ...
        <div class="mt-6 text-right">
            <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-carbon hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Guardar Cambios</button>
        </div>
    </form>

    <!-- Sección de Cambiar Contraseña -->
    <form id="form-change-password" @submit.prevent="CambiarContrasena()" x-ref="xPassForm" class="mb-8 p-6 border border-gray-200 rounded-lg shadow-sm">
    ...
    <form id="form-change-Gpassword" @submit.prevent="CambiarContrasenaG()" x-ref="xPassGForm" class="mb-8 p-6 border border-gray-200 rounded-lg shadow-sm">
        <?= csrf_field() ?>
