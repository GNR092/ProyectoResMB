        ...
        <div class="flex flex-col gap-1 self-end mb-0.5" x-show="departamentos.length > 0" x-cloak>
            <div class="flex items-center gap-3">
                <button @click="exportarAsignacionExcel()" 
                        title="Exportar presupuesto del mes seleccionado"
                        class="px-4 py-2 bg-green-600 text-white font-bold rounded-lg hover:bg-green-700 transition-all shadow-md flex items-center gap-2 border-b-4 border-green-800 active:border-b-0 active:translate-y-1">
                ...
                <button @click="exportarAnualExcel()" 
                        title="Exportar presupuesto de todo el año seleccionado"
                        class="px-4 py-2 bg-amber-500 text-white font-bold rounded-lg hover:bg-amber-600 transition-all shadow-md flex items-center gap-2 border-b-4 border-amber-700 active:border-b-0 active:translate-y-1">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div x-show="<?= (session('departamento_usuario') === 'Dirección') ? 'true' : 'false' ?> || usoCopia" class="flex gap-3">
                    <button @click="guardarMasivo(true)"
                            ...
                            class="px-4 py-2 bg-rose-600 text-white font-bold rounded-lg hover:bg-rose-700 transition-all shadow-md flex items-center gap-2 border-b-4 border-rose-800 disabled:opacity-50 disabled:border-b-0 disabled:cursor-not-allowed active:border-b-0 active:translate-y-1">
                        <span x-text="guardando ? 'Guardando...' : 'Asignar Resto del Año'"></span>
                    </button>
                    <button @click="guardarMasivo(false)"
                            :disabled="guardando || bloqueadoPorRevision"
                            class="px-6 py-2 bg-blue-700 text-white font-bold rounded-lg hover:bg-blue-800 transition-all shadow-md flex items-center gap-2 border-b-4 border-blue-900 disabled:opacity-50 disabled:border-b-0 disabled:cursor-not-allowed active:border-b-0 active:translate-y-1">

                        <svg x-show="guardando" class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
