    ...
    return {
        ...
        async cargardatos() {
            ...
            try {
                ...
                const detallesPromises = ordenes.map(o => SendDataEnd(`api/orden-compra/details/${o.ID_Solicitud}`));
                const detalles = await Promise.all(detallesPromises);

                if (!detalles) return;

                this.ordenesContado = detalles.filter(det => det && det.EstadoOrden === 'Espera_Programacion' && det.MetodoPago == '0');
                this.ordenesCredito = detalles.filter(det => det && det.EstadoOrden === 'Espera_Programacion' && det.MetodoPago == '1');
        ...
        generarDetalleHtml() {
            ...
            if (data.MetodoPago == 1) { // Solo mostrar si es de crédito
...
function ListaPagos() {
    return {
        ...
        init() {
            this.loading = true;
            SendDataEnd('api/pagos/programados')
                .then(data => {
                    this.pagos = data;
                    this.loading = false;
                })
                .catch(error => {
                    console.error('Error al cargar los pagos programados:', error);
                    this.loading = false;
                    mostrarNotificacion('No se pudieron cargar los pagos.', 'error');
                });
        },
        formatCurrency(value) {
            if (value === null || isNaN(value)) return 'N/A';
